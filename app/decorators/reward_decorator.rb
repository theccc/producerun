class RewardDecorator < Draper::Decorator
  ...
  def name
    ...
    %{
      <label data-minimum-value="#{source.minimum_value > 0 ? source.minimum_value.to_i : '1'}" class="w-form-label fontsize-large fontweight-semibold" for="contribution_reward#{source.id && "_#{source.id}"}">#{source.minimum_value > 0 ? source.display_minimum+' ' : I18n.t('rewards.index.dont_want')}</label>
      <div>
